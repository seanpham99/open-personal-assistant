...
C4Context
  ...
  Person(user, "User", "Interacts via CLI to manage tasks and agents.")
  ...
  Rel(user, assistant, "Uses", "CLI/HTTP")
...
C4Container
  ...
  Container_Boundary(pa_system, "Personal Assistant System") {
    Container(cli, "CLI Client", "TypeScript/Commander", "User interface for issuing commands and viewing status.")
  ...
  Container_Boundary(infra, "Infrastructure (Docker)") {
    ...
    Container(realtime, "Realtime Server", "Elixir/Phoenix", "Push notifications for CLI updates.")
  }

  Rel(user, cli, "Commands", "Shell")
  Rel(cli, daemon, "RPC/HTTP", "Localhost:3000")
...
C4Component
  ...
  Component(api_server, "API Server", "Fastify", "Exposes endpoints for CLI and potential future web UIs.")
